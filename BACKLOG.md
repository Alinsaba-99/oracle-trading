...
- [~] **BL-304** P2 — **Perpetual backfill execution**: lanciare backfill orchestrator su tutte le configurazioni in `backfill.conf`. AC: coverage > 90% degli asset listati entro 7gg. Vedi `data/lake/plans/backfill.conf`. 🔄 Stato 31-lug: curation 1m completa (58 serie, 181.7M righe, `b9c4742`); refresh perpetuo attivo (timer systemd 07:00 + `scripts/refresh_lake.py`); orchestrator con modalità `--incremental` (fetch solo barre mancanti da coverage.latest) e classificatore ok/fresh/failed (niente falsi failed nei weekend); fix API jetta: bucket correnti → HTTP 400, adapter clampa ai bucket chiusi (`_clamp_bucket_range`); piano live ripulito (152 entry raggiungibili: dukascopy+yahoo+binance; histdata/stooq/ibkr/databento bloccate e commentate — vedi `docs/DATA_SOURCES.md`). Verifica finale coverage in corso.
