...
print(f"  ConfigError:             [{err.code}] {err.args[0]!s}")
...
env = build_envelope(
    "market.bar", {"instrument": "SPY", "close": float(close[-1])}, source="showcase"
)
...
print("  Capitale iniziale:      $100,000")
...
print(
    # shortened(float(bt_result.final_equity) / 100000 - 1) * 100 - buy_hold:.1f}%"
)
...
from genetics.genome.signal import GenomeConfig, decode, encode, validate_genome
...
d = decode(g)
print("  Parametri GA:           10 (KNN + class_weight + HA)")
...
print("  GA run (pop=12, gen=12, 2-fold WFA):")
...
signals = [
    ...
    AnalystSignal(
        ...
        vote=AgentVote(
            direction="buy",
            confidence=0.55,
            reasoning="Trend rialzista SPY 2015-2020",
            risk_score=0.4,
        ),
    ...
    AnalystSignal(
        source="sentiment",
        vote=AgentVote(
            direction="hold", confidence=0.50, reasoning="Sentiment neutrale", risk_score=0.5
        ),
...
market_state = MarketState(
    regime="bull", phase="markup", volatility="medium", liquidity="normal", risk_appetite="risk_on"
)
...
print("  Segnali analyst:")
for s in signals:
    dir_icon = "▲" if s.vote.direction == "buy" else "▼" if s.vote.direction == "sell" else "◆"
    print(
        f"    {dir_icon} {s.source}: {s.vote.direction.upper()} (conf={s.vote.confidence:.2f}) — {s.vote.reasoning}"
    )
print(f"  Weighted vote:           BUY={buy_w:.2f} SELL={sell_w:.2f} HOLD={hold_w:.2f}")
print(
    f"  Decisione:              {decision.direction.upper()} ({decision.confidence:.1%} confidence)"
)
...
req = OrderRequest(instrument_id="SPY", side="buy", quantity=Decimal("100"), order_type="market")
result = None
import asyncio
...
print("  Paper Broker:           fills simulated con slippage 0.5%")
print(f"  Broker Registry:        {registry.list_brokers()}")
print("  Execution Algos:        VWAP, TWAP, Iceberg")
print("  IBKR Connector:         ib_insync 0.9.86 (TWS/IBGateway)")
print("  CCXT Connector:         ccxt 4.5.64 (100+ exchange)")
print("  MarketDataFeed:         volume profile 24h + bid/ask/last")
...
print("  Genetic Engine:         DEAP + NSGA-II a 4 obiettivi (Sharpe, Sortino, Calmar, MaxDD)")
print(f"  Genoma:                 {genome_config.n_params} parametri tipizzati (3 tipi)")
print("  Isole:                  Modello a isole asincrono con migrazione ring, checkpoint/restart")
...
print("    oracle agent run --instrument SPY")
print(
    "    python -m experiments.scripts.run_ga"
    " --symbol SPY --pop-size 100 --generations 50 --islands 4"
)
print("    oracle trade submit --instrument SPY --side buy --qty 100")
print()
