...
class TestCuratedAlphaLibrary:
    ...
    def test_categories_cover_all(self, library: CuratedAlphaLibrary, factor_names: list[str]) -> None:
    ...
    def test_batch_compute_empty_subset(self, library: CuratedAlphaLibrary, ohlcv: pl.DataFrame) -> None:
...
class TestFactorValues:
    ...
    def test_bb_position(self, library: CuratedAlphaLibrary) -> None:
        ...
        assert (last > 0.0).any(), f"BB position should be positive after price jump, got {last.to_list()}"
    ...
    def test_beta_sign(self, library: CuratedAlphaLibrary) -> None:
        ...
        last = result[-5:].fill_nan(0.0)
        assert (last > 0.0).any(), f"Beta should tend positive in trending market, got {last.to_list()}"
    ...
    def test_momentum_stability(self, library: CuratedAlphaLibrary) -> None:
        ...
        last = result[-5:].fill_nan(0.0)
        assert (last > 0.5).any(), f"Momentum stability should be high in uptrend, got {last.to_list()}"
...
class TestEdgeCases:
    ...
    def _make_data(self, n: int, close_val: float = 100.0, volume_val: int = 1_000_000) -> pl.DataFrame:
        close = [close_val] * n
        return pl.DataFrame({
            "timestamp": _timerange(n) if n > 0 else pl.Series("timestamp", [], dtype=pl.Datetime),
            "open": close,
            "high": close,
            "low": close,
            "close": close,
    ...
    def test_zero_volume(self, library: CuratedAlphaLibrary) -> None:
        ...
        vol_factors = ["volume_zscore_20", "turnover", "volume_vs_avg", "dollar_volume",
                        "amihud_illiquidity", "roll_impact"]
        for name in vol_factors:
            result = library.get(name)(data)
            assert isinstance(result, pl.Series)
            assert np.all(np.isfinite(result.fill_nan(0.0).to_numpy())), f"{name} has non-finite values"
    ...
    def test_very_large_values(self, library: CuratedAlphaLibrary) -> None:
        ...
        for name in library.factor_names:
            result = library.get(name)(data)
            assert isinstance(result, pl.Series)
            assert np.all(np.isfinite(result.fill_nan(0.0).to_numpy())), f"{name} has non-finite values with large prices"
