...
def _patch_db_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "apps.api.services.trade_service._DB_PATH",
        Path("/nonexistent/experiments.db"),
    )
    monkeypatch.setattr(
        "apps.api.services.checkpoint_reader.list_ga_runs",
        lambda: [],
    )
    monkeypatch.setattr(
        "apps.api.services.checkpoint_reader.get_ga_run",
        lambda _run_id: None,
    )
    monkeypatch.setattr(
        "apps.api.services.checkpoint_reader.get_latest_run_summary",
        lambda: None,
    )
    monkeypatch.setattr(
        "apps.api.services.checkpoint_reader.get_equity_curve",
        lambda: [],
    )
