...
class TestLossLimits:
    ...
    def test_daily_loss_pct_bounds(
        self, gov: PropFirmRiskGovernor, loss_pct: float
    ) -> None:
    ...
    @pytest.mark.parametrize("loss_pct", [0.01, 0.05, 0.10, 0.25, 0.50])
    def test_overall_loss_pct_bounds(
        self, gov: PropFirmRiskGovernor, loss_pct: float
    ) -> None:
...
class TestMaxPositionSize:
    ...
    def test_max_position_size_positive(
        self, gov: PropFirmRiskGovernor
    ) -> None:
        """Max position size must always be non-negative."""
        size = gov.max_position_size(
            entry=100.0, stop=99.0, contract_size=100
        )
        assert size >= 0

    def test_larger_stop_reduces_size(
        self, gov: PropFirmRiskGovernor
    ) -> None:
        """Wider stop distance means smaller position."""
        tight_stop = gov.max_position_size(
            entry=100.0, stop=99.0, contract_size=100
        )
        wide_stop = gov.max_position_size(
            entry=100.0, stop=95.0, contract_size=100
        )
        assert wide_stop <= tight_stop, "Wider stop should not increase position"

    def test_larger_contract_reduces_size(
        self, gov: PropFirmRiskGovernor
    ) -> None:
        """Larger contract multiplier means smaller position."""
        small_contract = gov.max_position_size(
            entry=100.0, stop=99.0, contract_size=100
        )
        large_contract = gov.max_position_size(
            entry=100.0, stop=99.0, contract_size=1000
        )
    ...
    def test_zero_stop_distance_returns_zero(
        ...
        size = gov.max_position_size(
            entry=100.0, stop=100.0, contract_size=100
        )
        assert size == 0.0
...
class TestCheckNewOrder:
    ...
    def test_denied_when_daily_loss_exceeded(
        self, gov: PropFirmRiskGovernor
    ) -> None:
        ...
        check = gov.check_new_order(
            entry=100.0, stop=99.0, lots=1.0, contract_size=100_000
        )
        assert not check.allowed

    def test_denied_when_overall_loss_exceeded(
        self, gov: PropFirmRiskGovernor
    ) -> None:
...
class TestRollover:
    ...
    def test_daily_pnl_resets_on_rollover(
        self, gov: PropFirmRiskGovernor
    ) -> None:
    ...
    def test_balance_preserved_on_rollover(
        self, gov: PropFirmRiskGovernor
    ) -> None:
...
class TestChallengeOutcome:
    ...
    def test_initial_is_in_progress(
        self, gov: PropFirmRiskGovernor
    ) -> None:
        assert gov.challenge_outcome().value == "in_progress"

    def test_profit_target_ends_challenge(
        self, gov: PropFirmRiskGovernor
    ) -> None:
    ...
    def test_overall_loss_fails_challenge(
        self, gov: PropFirmRiskGovernor
    ) -> None:
...
class TestCrossProfile:
    ...
    def test_different_profiles_have_different_limits(
        ...
        assert gov.profile.max_daily_loss_pct != topstep_gov.profile.max_daily_loss_pct or \
               gov.profile.max_overall_loss_pct != topstep_gov.profile.max_overall_loss_pct
